...
We're setting up the extensions so they will automatically use the code that is established to work for different core versions, so in theory they will always be backward-compatible, even if there are drastic changes to the core. We're looking at the cheapest subscription model for those, as doing all this for free just sets us up for going down in history as martyrs to corporations who will take our ideas and profit financially from them, and it would make sense for them to do so. We don't get a penny for doing this - we have some side gigs as well as our work on Cause. So financial incentive would help us to devote as much as we can on core and extension work. We will be publishing the source code for the extensions at some point soon though regardless, to give other people guidance on writing extensions for Cause too.<br>

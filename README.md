...
This will include JavaScript expressions, scoped variables, the ability to create commands on demand in the Cause config itself, variable binding and event triggering on variable/attribute change, the equivalent of setInterval for repeat events, labelling of individual actions for isolated delay cancelling, custom element attribute binding, babel versions of the core, international character set support, shadow DOM, easy variable scoping to the document or individual shadow DOMs (without the need for state management tools as it's frickin obvious to track variables in a document or component scope or you shouldn't be coding at all), components within components, and a couple of other things, including @for looping! We've fixed a few bugs too.<br>

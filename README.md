...
News:<br>
After getting some feedback from the community, we have completed a major overhaul to the development process so we can release the code as fully open-source (ie. not minified - so you can fork it properly) on the MIT license and switch to a donation model. This will go live with the next release. To test the new flow out in practice, before we publish for real, a large update is coming - with support for JavaScript expression variables and the ability to create custom commands and JavaScript functions without leaving the config, plus easy-to-use commands to handle the now widely supported Web Components API, so that's shadow DOM and all that other weird stuff. This is stuff that isn't on the published roadmap, but is really important as components will get more trendy soon, and should be publishable in the next week or so (add on time for the extensions to get approved). Stay tuned!
